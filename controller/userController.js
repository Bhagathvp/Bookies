...
const getIndex = async(req,res)=>{
    ...
    try{ res.render("users/index", {product: product, banner: banners,id:id})}
  ...
  const twoFactor = async (req, res) => {
    try {
        if (req.query.id == req.body.otp) {
            ...
            res.render('users/index', { user: req.session.user, product: product,banner:banners, id:req.session.user_id });
        }
